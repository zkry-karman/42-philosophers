...
int	are_all_full(t_philo *philo)
{
	...
	if (philo[0].data->num_times_to_eat < 1)
		return (0);
	j = 0;
	all_full = 1;
	while (j < philo[0].data->number_of_philo)
	{
		if (philo[j].meals_eaten < philo[0].data->num_times_to_eat)
		{
			all_full = 0;
			break ;
		}
...
void	*check_for_deaths(void *arg)
{
	...
	while (1)
	{
		...
		{
			pthread_mutex_lock(&philo[0].data->dead_lock);
			if (get_time() - philo[i].last_meal >= philo[0].data->time_to_die)
			{
				print_deathstamp(philo);
				return (NULL);
			}
			pthread_mutex_unlock(&philo[0].data->dead_lock);
			i++;
